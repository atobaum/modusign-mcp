...
export function registerFileTools(server: McpServer, client: ModusignClient): void {
  ...
  server.tool(
    'file_upload',
    'Upload a file (base64 encoded) for use in document creation. Returns fileId + token valid for 2 hours. 파일을 업로드합니다. 반환된 fileId와 token은 2시간 유효하며, document_create에서 사용할 수 있습니다. Supported formats: pdf, hwp, hwpx, doc, docx, xls, xlsx, ppt, pptx, bmp, gif, jpg, jpeg, png, tiff.',
    {
  ...
  server.tool(
    'file_merge',
    'Merge multiple uploaded PDF files into a single file. Use file_upload first to get fileId+token for each file. 여러 PDF 파일을 하나로 병합합니다. 먼저 file_upload로 각 파일의 fileId+token을 얻어야 합니다.',
    {
